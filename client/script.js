...
function changeBtnColorAndText(event) {
  event.target.style.backgroundColor = "black";
  event.target.style.color = "white";
}

sayHelloButton.addEventListener("mouseover", changeBtnColorAndText);
...
function revertBtnColorAndText(event) {
  event.target.style.backgroundColor = "white";
  event.target.style.color = "black";
}

sayHelloButton.addEventListener("mouseout", revertBtnColorAndText);
...
const repeatMyParam = (param) => {
  ...
  axios
    ...
    .then((repsonse) => console.log(response.data));
...
function queryRequest(param) {
  axios
    .get(`http://localhost:3000/query-test?${param}`)
